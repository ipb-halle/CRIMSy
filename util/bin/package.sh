...
function copyFiles {
    ...
    grep STABLE config/revision_info.cfg | tail -1 | cut -d';' -f1 > target/dist/etc/revision_info.cfg
...
case $2 in
    ...
    MASTERBATCH)
        BATCH="--batch"
        tmp=`grep -vE "^#" $LBAC_REPO/util/test/etc/cloudnodes.cfg | grep -E "^$LBAC_CLOUD;" | grep MASTER | cut -d';' -f2`
